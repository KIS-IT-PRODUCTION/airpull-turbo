    ...
    <section className="bg-black py-24 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-end justify-between mb-12">
          ...
          <Link
            href="/catalog"
            className="hidden md:inline-flex items-center gap-2 text-white/60 hover:text-white transition-colors font-medium"
          >
            Всі товари →
          </Link>
        </div>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {products.map((product) => (
              <div
                key={product.id}
                className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-violet-500/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-violet-500/10"
              >
                {/* Фото */}
                <div className="h-52 bg-gradient-to-br from-violet-900/40 to-pink-900/40 flex items-center justify-center overflow-hidden">
                  {product.imageUrl ? (
                    <img
        ...
        <div className="text-center mt-10 md:hidden">
          <Link
            href="/catalog"
            className="inline-flex px-6 py-3 rounded-full border border-white/20 text-white/70 hover:text-white hover:border-white/40 transition-all font-medium"
          >
            Всі товари →
          </Link>
        </div>

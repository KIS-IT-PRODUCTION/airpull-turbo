...
// Копіюємо типи з твоєї сторінки
interface UserInfo {
  id: string;
  name: string | null;
  phone: string;
...
export default function OrdersTableClient({ initialOrders }: OrdersTableClientProps) {
  // Стан для активної вкладки фільтру
  const [activeTab, setActiveTab] = useState<string>('ALL');

  // Фільтруємо замовлення залежно від вибраної вкладки
  const filteredOrders = initialOrders.filter((order) => {
    if (activeTab === 'ALL') return true;
    return order.status === activeTab;
  });

  // Групуємо відфільтровані замовлення по днях
  ...
  // Список вкладок для відображення
  const tabs = [
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex space-x-1 bg-[#1a1a1a] p-1 rounded-xl border border-white/10 overflow-x-auto hide-scrollbar">
        {tabs.map((tab) => (
          <button
            key={tab.id}
          ...
          >
            {tab.label}
            {/* Лічильник замовлень для вкладки */}
            <span className={`ml-2 px-2 py-0.5 rounded-md text-xs ${activeTab === tab.id ? 'bg-violet-500/30' : 'bg-white/10'}`}>
              {tab.id === 'ALL' 
                ? initialOrders.length 
                : initialOrders.filter(o => o.status === tab.id).length}
            </span>
          </button>
        ))}
      </div>

      {/* 🟢 Таблиця з групуванням по днях */}
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-white/50">
                    <div className="flex flex-col items-center justify-center space-y-3">
                      ...
                      <span>У цій категорії немає замовлень</span>
                    </div>
                  </td>
                </tr>
              ) : (
                // Рендеримо групи по днях
                Object.entries(groupedOrders).map(([date, dayOrders]) => (
                  <React.Fragment key={date}>
                    {/* Рядок-розділювач для дати */}
                    <tr className="bg-[#222222]">
                      <td colSpan={6} className="px-6 py-2 text-xs font-bold text-white/70 uppercase tracking-wider">
                        📅 {date}
                      </td>
                    </tr>
                    
                    {/* Самі замовлення за цей день */}
                    {dayOrders.map((order) => {
